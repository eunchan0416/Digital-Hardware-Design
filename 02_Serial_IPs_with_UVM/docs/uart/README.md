...
* **Baud Rate Generation:** Configurable via hardware parameters. Employs a 16x baud rate clock generation for stable mid-bit alignment.
...
### 4.1. Test Scenarios & Scoreboard Predictor
* **Randomized Payload Generation:** The UVM Sequencer generates constraint-random 8-bit payloads for both RX and TX paths.
* **Transaction Matching:** The `uart_scoreboard` employs asynchronous FIFOs to capture expected transactions directly from the driver/sequencer and compares them against actual transactions captured by the physical line monitors.
* **Test Iterations:** The base test successfully executed **10 consecutive random RX operations** followed by **10 consecutive random TX operations** without any data loss or misalignment.

### 4.2. Hardware Timing Verification (Waveform Proof)
